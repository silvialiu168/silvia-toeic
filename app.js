const DATA="data/", DATA_VERSION="38", $=s=>document.querySelector(s);
const state={users:[],user:null,questions:[],knowledge:[],vocab:[],pastPapers:null,gsatReview:[],gsatReviewIndex:0,stats:null,vocabStats:null,current:null,vocabQuestion:null,grammarMode:"smart",vocabMode:"en-zh"};
...
async function init(){
  ...
  state.users=await json("users.json");migrateOldSilviaData();setupNav();setupInstall();await activateUser(state.users[0],false);
}
...
async function activateUser(u,openLearn=true){
  state.user=u;({stats:state.stats,vocab:state.vocabStats}=userData(u));const knowledge=[...await json("knowledge_common.json"),...await json(`knowledge_${u.level}.json`)];if(u.level==="junior")knowledge.push(...await json("knowledge_junior_past.json"));state.knowledge=[...new Map(knowledge.map(k=>[k.id,k])).values()];const baseQuestions=await json(`questions_${u.level}.json`),pastQuestions=u.level==="junior"?(await json("questions_junior_past.json")).questions.filter(q=>q.practiceReady!==false):[];state.questions=[...pastQuestions,...baseQuestions].filter(isTrustedQuestion).map(q=>({...q,knowledgePoint:q.knowledgePoint||q.knowledge_id||q.knowledgeId,knowledge_id:q.knowledge_id||q.knowledgePoint||q.knowledgeId}));state.vocab=(await json(`vocab_${u.level}.json`)).filter(v=>v.practiceReady!==false);state.pastPapers=u.level==="junior"?await json("past_papers_junior.json"):null;state.gsatReview=u.level==="gsat"?await json("gsat_115_question_sample.json"):[];
  $("#pageTitle").textContent=`${u.name} · ${u.goal}`;
  ["learnPage","mistakesPage","growthPage","familyPage"].forEach(id=>$("#"+id).innerHTML="");
  renderDashboard();showPage(openLearn?"learnPage":"dashboardPage");
}
...
function showPage(id){$("#familyView").classList.add("hidden");$("#learnerView").classList.remove("hidden");document.querySelectorAll(".page").forEach(x=>x.classList.toggle("active",x.id===id));document.querySelectorAll("#bottomNav button").forEach(x=>x.classList.toggle("active",x.dataset.page===id));$("#pageTitle").textContent=id==="dashboardPage"?"Family English Challenge":`${state.user.name} · ${state.user.goal}`;if(id==="dashboardPage")renderDashboard();else if(id==="learnPage"&&!$("#learnPage").innerHTML)renderLearn();else if(id==="mistakesPage")renderMistakes();else if(id==="growthPage")renderGrowth();else if(id==="familyPage")renderFamily()}
...
function renderLearn(){
  ...
  $("#learnPage .tabs").onclick=e=>{const b=e.target.closest("button");if(!b)return;$("#learnPage").querySelectorAll(".tabs button").forEach(x=>x.classList.toggle("active",x===b));$("#learnPage").querySelectorAll(".learn-section").forEach(x=>x.classList.toggle("active",x.id===b.dataset.tab));if(b.dataset.tab==="grammar")renderGrammar();if(b.dataset.tab==="vocab")renderVocab();if(b.dataset.tab==="reading")renderReading()};
  renderGrammar();
}
...
function renderVocab(){
  const modes={"en-zh":"英文選中文","zh-en":"中文選英文",sentence:"例句填空",pos:"詞性判斷",forms:"詞形變化"};
  $("#vocab").innerHTML=`${progressCard("vocab")}<div class="chips practice-modes">${Object.entries(modes).map(([m,l])=>`<button type="button" class="chip ${state.vocabMode===m?"active":""}" data-vmode="${m}">${l}</button>`).join("")}</div><div id="vocabPractice"></div><button type="button" class="secondary vocab-book-toggle" id="toggleVocabBook">展開單字本（${state.vocab.length}）</button><div class="vocab-list hidden" id="vocabBook"></div>`;
  $("#vocab .chips").onclick=e=>{const b=e.target.closest("[data-vmode]");if(!b)return;state.vocabMode=b.dataset.vmode;renderVocab()};
  $("#toggleVocabBook").onclick=()=>{
    const book=$("#vocabBook"),hidden=!book.classList.contains("hidden");
...
function renderReading(){
  if(state.user.level==="gsat"&&state.gsatReview.length){
    $("#reading").innerHTML=`<section class="panel gsat-review-intro"><h3>115 學測英文題庫驗收體驗</h3><p>這裡使用官方 115 學年度英文題本建立樣板，共 ${state.gsatReview.length} 筆。所有題目仍標記為待審核，讓你先像使用者一樣體驗流程。</p><div class="stats-grid"><div class="stat"><span>選擇／混合題</span><strong>1-50</strong></div><div class="stat"><span>非選擇題</span><strong>3</strong></div></div><button type="button" class="primary" id="startGsatReview">開始體驗</button></section><div id="gsatReviewBox"></div>`;
    $("#startGsatReview").onclick=()=>{state.gsatReviewIndex=0;renderGsatReview()};
    return;
  }
...
function gsatOptions(item){return Object.entries(item.options||{}).filter(([,v])=>String(v||"").trim()).map(([k,v])=>({key:k,label:v}))}
function renderGsatReview(){
  const item=state.gsatReview[state.gsatReviewIndex],opts=gsatOptions(item),answer=String(item.answer||""),canAnswer=opts.length&&answer.length===1;
  $("#gsatReviewBox").innerHTML=`<article class="card gsat-review-card"><div class="question-meta"><span class="chip">115 學測 · ${item.section}</span><span class="chip review-chip">待審核</span><span class="chip">${state.gsatReviewIndex+1} / ${state.gsatReview.length}</span></div><h3>${item.id}</h3>${item.context?`<details class="gsat-context" open><summary>查看題組文章</summary><p>${item.context}</p></details>`:""}<p class="question">${item.question}</p>${item.assets?.length?`<div class="visual-warning">這題含圖片或特殊版面，請對照 <a href="official/gsat/115/question.pdf#page=${item.source?.page||1}" target="_blank">官方 PDF 第 ${item.source?.page||"?"} 頁</a>。</div>`:""}${opts.length?`<div class="options">${opts.map(o=>`<button type="button" class="option" data-key="${o.key}">${o.key}. ${o.label}</button>`).join("")}</div>`:`<div class="details">這題不是一般選擇題。${answer?`官方答案：${answer}`:"答案需搭配官方評分標準確認。"}</div>`}<button type="button" class="primary submit-answer" id="submitGsatReview" ${canAnswer?"disabled":"hidden"}>請選擇一個答案</button><div id="gsatReviewResult"></div><div class="review-nav"><button type="button" class="secondary" id="prevGsatReview" ${state.gsatReviewIndex===0?"disabled":""}>上一題</button><button type="button" class="primary" id="nextGsatReview">${state.gsatReviewIndex===state.gsatReview.length-1?"完成":"下一題"}</button></div></article>`;
  $("#prevGsatReview").onclick=()=>{state.gsatReviewIndex=Math.max(0,state.gsatReviewIndex-1);renderGsatReview()};
  $("#nextGsatReview").onclick=()=>{state.gsatReviewIndex=Math.min(state.gsatReview.length-1,state.gsatReviewIndex+1);renderGsatReview()};
  const submit=$("#submitGsatReview");
  $("#gsatReviewBox .options")?.addEventListener("click",e=>{const b=e.target.closest(".option");if(!b||!canAnswer)return;$("#gsatReviewBox").querySelectorAll(".option").forEach(x=>x.classList.toggle("selected",x===b));submit.disabled=false;submit.textContent="提交答案";submit.dataset.selected=b.dataset.key});
  submit?.addEventListener("click",()=>{const selected=submit.dataset.selected,ok=selected===answer;$("#gsatReviewBox").querySelectorAll(".option").forEach(b=>{b.disabled=true;b.classList.toggle("correct",b.dataset.key===answer);b.classList.toggle("wrong",b.dataset.key===selected&&!ok)});submit.classList.add("hidden");$("#gsatReviewResult").innerHTML=`<div class="result ${ok?"good":"bad"}">${ok?"答對":"答錯"} · 正確答案 ${answer}</div><div class="details">這是驗收體驗題，不會寫入正式學習紀錄。</div>`});
}
