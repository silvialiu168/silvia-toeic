...
function renderGrammar(){
  ...
  $("#grammar").innerHTML=`<div class="chips">${Object.entries(labels).map(([m,l])=>`<button type="button" class="chip ${state.grammarMode===m?"active":""}" data-mode="${m}">${l}</button>`).join("")}</div><article class="card practice-card"><span class="chip">${q.category}</span><p class="question">${q.question}</p><div class="options">${q.options.map((o,i)=>`<button type="button" class="option" data-i="${i}">${String.fromCharCode(65+i)}. ${o}</button>`).join("")}</div><button type="button" class="primary submit-answer" id="submitGrammar" disabled>请选择一个答案</button><div id="grammarResult"></div></article>`;
...
function answerGrammar(i){
  ...
  $("#grammarResult").innerHTML=`<div class="result ${ok?"good":"bad"}">${ok?"答对":"答错"} · 正确答案 ${q.options[q.answer]} · 累计已做 ${state.stats.attempts} 题</div><div class="actions"><button type="button" class="secondary" id="teacher">Silvia 老师讲题</button><button type="button" class="primary" id="nextG">下一题</button></div><div id="analysis" class="teacher-analysis hidden">${teacherAnalysis(q,i,ok)}</div>`;$("#teacher").onclick=()=>$("#analysis").classList.toggle("hidden");$("#nextG").onclick=()=>{delete $("#grammar").dataset.locked;delete $("#grammar").dataset.selected;renderGrammar()};
}
...
function renderVocab(){const modes={"en-zh":"英文选中文","zh-en":"中文选英文",sentence:"例句填空",pos:"词性判断",forms:"词形变化"};$("#vocab").innerHTML=`<div class="chips practice-modes">${Object.entries(modes).map(([m,l])=>`<button type="button" class="chip ${state.vocabMode===m?"active":""}" data-vmode="${m}">${l}</button>`).join("")}</div><div id="vocabPractice"></div><button type="button" class="secondary vocab-book-toggle" id="toggleVocabBook">展开单词本（${state.vocab.length}）</button><div class="vocab-list hidden" id="vocabBook">${state.vocab.map(v=>`<div class="vocab-item"><strong>${v.word}</strong><span>${v.chinese}</span><small>${status(state.vocabStats.words[v.id])} · ${state.vocabStats.words[v.id]?.accuracy||0}%</small></div>`).join("")}</div>`;$("#vocab .chips").onclick=e=>{const b=e.target.closest("[data-vmode]");if(!b)return;state.vocabMode=b.dataset.vmode;renderVocab()};$("#toggleVocabBook").onclick=()=>{const book=$("#vocabBook"),hidden=book.classList.toggle("hidden");$("#toggleVocabBook").textContent=hidden?`展开单词本（${state.vocab.length}）`:"收起单词本"};vocabQuestion()}
...
function vocabQuestion(){
  ...
  state.vocabQuestion={v,correct,answered:false,selected:null};$("#vocabPractice").innerHTML=`<article class="card practice-card"><span class="chip">${state.vocabMode}</span><p class="question">${prompt}</p><div class="options">${shuffle(opts).map(x=>`<button type="button" class="option">${x}</button>`).join("")}</div><button type="button" class="primary submit-answer" id="submitVocab" disabled>请选择一个答案</button><div id="vResult"></div></article>`;$("#vocabPractice .options").onclick=e=>{const b=e.target.closest(".option");if(!b||state.vocabQuestion.answered)return;$("#vocabPractice").querySelectorAll(".option").forEach(x=>x.classList.toggle("selected",x===b));state.vocabQuestion.selected=b.textContent;$("#submitVocab").disabled=false;$("#submitVocab").textContent="提交答案"};$("#submitVocab").onclick=()=>{if(state.vocabQuestion.selected===null||state.vocabQuestion.answered)return;state.vocabQuestion.answered=true;$("#submitVocab").disabled=true;answerVocab(state.vocabQuestion.selected)}
}
function answerVocab(a){const{v,correct}=state.vocabQuestion,ok=a===correct,s=state.vocabStats.words[v.id]||{attempts:0,correct:0,wrong:0};s.attempts++;ok?s.correct++:s.wrong++;s.accuracy=Math.round(s.correct/s.attempts*100);s.mastery=s.accuracy;s.last_practiced=new Date().toISOString();state.vocabStats.words[v.id]=s;updateStreak();save();renderDashboard();renderGrowth();renderFamily();const f=formsFor(v);$("#vocabPractice").querySelectorAll(".option").forEach(b=>{b.disabled=true;b.classList.toggle("correct",b.textContent===correct);b.classList.toggle("wrong",b.textContent===a&&!ok)});$("#submitVocab").classList.add("hidden");$("#vResult").innerHTML=`<div class="result ${ok?"good":"bad"}">${ok?"答对":"答错"} · ${correct}</div><div class="details">${v.word}：${v.chinese}<br>词性：${(v.part_of_speech||[]).join(" / ")}<br>词形：${f.base} / ${f.third_person} / ${f.past} / ${f.past_participle} / ${f.ing}<br>${v.examples[0].sentence}<br>常见搭配：${(v.common_collocations||[]).join("；")}<br>${v.common_error}</div><button type="button" class="primary" id="nextV">下一词</button>`;$("#nextV").onclick=vocabQuestion}
